...
const ProductivityGoalSchema = new mongoose.Schema({
    ...
    goalStatus: {
        type: String,
        enum: ['active', 'completed', 'behind_schedule', 'ahead_of_schedule', 'abandoned'],
        default: 'active'
    },
...
ProductivityGoalSchema.pre('save', function(next) {
    if (this.targetMetrics.currentValue && this.targetMetrics.targetValue) {
        ...
        } else if (this.progressData.completionPercentage < 50 && 
...
ProductivityGoalSchema.methods.updateProgress = function(newValue, notes = '') {
