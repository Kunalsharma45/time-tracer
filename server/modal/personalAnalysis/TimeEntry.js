...
TimeEntrySchema.pre('save', function(next) {
    if (this.endTimestamp && this.startTimestamp) {
        const timeDifferenceInMilliseconds = this.endTimestamp - this.startTimestamp;
        ...
        if (this.interruptionLogs && this.interruptionLogs.length > 0) {
            this.interruptionLogs.forEach(interruption => {
                if (interruption.endTimestamp && interruption.startTimestamp) {
                    const interruptionMilliseconds = interruption.endTimestamp - interruption.startTimestamp;
                    interruption.durationInMinutes = Math.round(interruptionMilliseconds / 60000);
                }
            });
        }
    }
...
TimeEntrySchema.methods.getNetProductiveTime = function() {
    ...
    if (this.interruptionLogs && this.interruptionLogs.length > 0) {
        totalInterruptionTime = this.interruptionLogs.reduce((total, interruption) => {
