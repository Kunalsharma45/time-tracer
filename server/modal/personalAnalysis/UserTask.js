...
UserTaskSchema.pre('save', function(next) {
    ...
    if (this.productivityMetrics.totalTimeSpentInMinutes > 0 && this.estimatedDurationInMinutes > 0) {
        this.productivityMetrics.completionEfficiency = 
            (this.estimatedDurationInMinutes / this.productivityMetrics.totalTimeSpentInMinutes) * 100;
    }
    
    // Calculate estimation accuracy
    if (this.estimatedDurationInMinutes > 0 && this.productivityMetrics.totalTimeSpentInMinutes > 0) {
        const difference = Math.abs(this.productivityMetrics.totalTimeSpentInMinutes - this.estimatedDurationInMinutes);
        this.productivityMetrics.estimatedAccuracy = 
            ((this.estimatedDurationInMinutes - difference) / this.estimatedDurationInMinutes) * 100;
    }
...
UserTaskSchema.methods.updateProgress = async function(progressData) {
    ...
    if (focusScore !== undefined) {
        const currentTotal = this.productivityMetrics.averageFocusScore * 
...
UserTaskSchema.statics.getUserTaskSummary = async function(userId) {
    ...
    const summary = {
        ...
        totalEstimatedTime: tasks.reduce((sum, task) => sum + (task.estimatedDurationInMinutes || 0), 0),
        totalActualTime: tasks.reduce((sum, task) => sum + (task.productivityMetrics.totalTimeSpentInMinutes || 0), 0),
    ...
    tasks.forEach(task => {
        ...
        summary.byPriority[task.priorityLevel] = (summary.byPriority[task.priorityLevel] || 0) + 1;
    });
    ...
    if (completedTasks.length > 0) {
        const totalScore = completedTasks.reduce((sum, task) => 
            sum + task.getProductivityScore(), 0);
        summary.averageProductivityScore = Math.round(totalScore / completedTasks.length);
    }
...
UserTaskSchema.index({ userId: 1, isRecurring: 1, 'recurrencePattern.nextOccurrenceDate': 1 });
