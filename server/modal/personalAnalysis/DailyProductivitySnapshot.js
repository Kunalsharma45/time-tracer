...
DailyProductivitySnapshotSchema.pre('save', function(next) {
    ...
    if (this.totalTimeTrackedInMinutes > 0) {
        this.efficiencyRatio = this.netProductiveTimeInMinutes / this.totalTimeTrackedInMinutes;
    }
    
    // Calculate task completion percentage
    if (this.totalTasksAssigned > 0) {
        this.taskCompletionPercentage = (this.tasksCompleted / this.totalTasksAssigned) * 100;
    }
...
DailyProductivitySnapshotSchema.index({ userId: 1, 'calendarData.weekNumber': 1, 'calendarData.year': 1 });
DailyProductivitySnapshotSchema.index({ userId: 1, 'calendarData.month': 1, 'calendarData.year': 1 });
...
const DailyProductivitySnapshot = mongoose.model('DailyProductivitySnapshot', DailyProductivitySnapshotSchema);
