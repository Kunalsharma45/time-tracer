    ...
    try {
        ...
        if (activeEntry) {
            return res.status(400).json({
                success: false,
                error: 'Validation Error',
                message: 'You already have an active time entry. Please stop it before starting a new one.',
                data: {
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    efficiency: updatedEntry.durationInMinutes > 0 ? 
                        (netProductiveTime / updatedEntry.durationInMinutes).toFixed(2) : 0
                }
            }
        });
    ...
    try {
        ...
        const processedEntries = timeEntries.map(entry => ({
            ...
            netProductiveTime: includeInterruptions ? 
                (entry.durationInMinutes - (entry.interruptionLogs?.reduce((sum, i) => sum + (i.durationInMinutes || 0), 0) || 0)) : 
                undefined,
...
export const logInterruption = async (req, res) => {
    try {
        ...
        res.status(201).json({
            ...
            data: {
                interruptionId: timeEntry.interruptionLogs[timeEntry.interruptionLogs.length - 1]._id,
