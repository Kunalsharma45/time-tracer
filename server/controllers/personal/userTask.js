...
export const createUserTask = async (req, res) => {
    try {
        ...
        const validCategories = [
            'academic_studies', 'assignment_work', 'project_development', 
            'exam_preparation', 'research_work', 'skill_learning',
            'personal_development', 'health_fitness', 'social_activities',
...
export const getUserTasks = async (req, res) => {
    try {
        ...
        const processedTasks = tasks.map(task => ({
            ...
            isOverdue: new Date(task.deadlineDate) < new Date() && task.taskStatus !== 'completed',
            daysUntilDeadline: task.deadlineDate ? 
                Math.ceil((new Date(task.deadlineDate) - new Date()) / (1000 * 60 * 60 * 24)) : null,
...
export const updateTaskProgress = async (req, res) => {
    try {
        const userId = req.user.id;
        const { taskId } = req.params;
        const { timeSpentInMinutes, focusScore, isCompleted } = req.body;
        ...
        res.json({
            ...
            data: {
                task: {
                    ...
                    totalTimeSpent: updatedTask.productivityMetrics.totalTimeSpentInMinutes,
    ...
    try {
        ...
        const analytics = {
            ...
            performanceInsights: {
                ...
                timeDifference: task.productivityMetrics.totalTimeSpentInMinutes - task.estimatedDurationInMinutes,
                timeDifferencePercentage: task.estimatedDurationInMinutes > 0 ? 
                    ((task.productivityMetrics.totalTimeSpentInMinutes - task.estimatedDurationInMinutes) / task.estimatedDurationInMinutes) * 100 : 0,
                
                // Focus consistency
                focusConsistency: task.productivityMetrics.averageFocusScore >= 3.5 ? 'good' : 
                                 task.productivityMetrics.averageFocusScore >= 2.5 ? 'average' : 'needs_improvement',
                
                // Efficiency rating
                efficiencyRating: task.productivityMetrics.completionEfficiency > 120 ? 'excellent' :
                                task.productivityMetrics.completionEfficiency > 90 ? 'good' :
                                task.productivityMetrics.completionEfficiency > 70 ? 'average' : 'below_average'
            },
        ...
        if (task.productivityMetrics.totalTimeSpentInMinutes > task.estimatedDurationInMinutes * 1.5) {
            analytics.recommendations.push({
                type: 'time_management',
                message: 'This task is taking significantly longer than estimated. Consider breaking it down into smaller subtasks.',
                priority: 'high'
            });
        }
        ...
        if (task.productivityMetrics.averageFocusScore < 2.5) {
            analytics.recommendations.push({
                ...
                message: 'Low focus scores detected. Try working in focused time blocks with fewer distractions.',
                priority: 'medium'
            });
        }
        ...
        if (task.isOverdue && task.taskStatus !== 'completed') {
            analytics.recommendations.push({
                ...
                message: 'Task is overdue. Consider renegotiating the deadline or prioritizing this task.',
                priority: 'critical'
            });
        }
